...
if(isset($_GET['email'])){
	$query=mysqli_query($connect,'SELECT * FROM digi_card WHERE user_email="'.$_GET['email'].'" AND user_active="NO" ');
		if(!mysqli_num_rows($query)==0){
			//login function 
			$row=mysqli_fetch_array($query);
			
			if($row['user_token']==$_GET['token'] ){
				$update=mysqli_query($connect,'UPDATE digi_card SET user_active="YES" WHERE user_email="'.$_GET['email'].'" ');
