...
<body>
    ...
    <div>
        <div class="card" id="home">
            ...
            <div class="card_content">
                <img src="<?php if (!empty($row['d_logo'])) {
                                echo 'data:image/*;base64,' . base64_encode($row['d_logo']);
                            } ?>"
                    alt="Logo">
            </div>
            <div class="card_content2">                
                <h2><?php if (!empty($row['d_f_name'])) {
                        echo $row['d_f_name'] . ' ' . $row['d_l_name'];
                    } ?></h2>
                <p><?php if (!empty($row['d_position'])) {
                        echo $row['d_position'];
                    } ?></p>
                    
                <p><?php if (!empty($row['d_comp_name'])) {
                        echo $row['d_comp_name'];
                    } ?></p>

            </div>
            <div class="dis_flex">
                <?php if (!empty($row['d_contact'])) {
                    echo '<a href="tel:+91' . $row['d_contact'] . '" target="_blank"><div class="link_btn"><i class="fa fa-phone"></i> Call</div></a>';
                } ?>
                <?php if (!empty($row['d_whatsapp'])) {
                    echo '<a href="https://api.whatsapp.com/send?phone=91' . str_replace('+91', '', $row['d_whatsapp']) . '&text=Hi, ' . $row['d_comp_name'] . '" target="_blank"><div class="link_btn"><i class="fa fa-whatsapp"></i> WhatsApp</div></a>';
                } ?>



                <?php if (!empty($row['d_location'])) {
                    echo '<a href="' . $row['d_location'] . '" target="_blank"><div class="link_btn"><i class="fa fa-map-marker"></i> Direction</div></a>';
                } ?>
                <?php if (!empty($row['user_email'])) {
                    echo '<a href="Mailto:' . $row['user_email'] . '" target="_blank"><div class="link_btn"><i class="fa fa-envelope"></i> Mail</div></a>';
                } ?>
                <?php if (!empty($row['d_website'])) {
                    echo '<a href="https://' . $row['d_website'] . '" target="_blank"><div class="link_btn"><i class="fa fa-globe"></i> Website</div></a>';
                } ?>
            ...
            <div class="contact_details">
                <?php if (!empty($row['d_contact'])) {
                    echo '<div class="contact_d"><i class="fa fa-phone"></i><p>' .$row['d_country_code'].'-'.$row['d_contact'] . '</p></div>';
                } ?>
                <?php if (!empty($row['d_contact2'])) {
                    echo '<div class="contact_d"><i class="fa fa-phone"></i><p>' .$row['d_country_code'].'-'.$row['d_contact2'] . '</p></div>';
                } ?>
                <?php if (!empty($row['user_email'])) {
                    echo '<div class="contact_d"><i class="fa fa-envelope"></i><p>' . $row['user_email'] . '</p></div>';
                } ?>
                <?php if (!empty($row['d_address'])) {
                    echo '<div class="contact_d"><i class="fa fa-map-marker" ></i><p>' . $row['d_address'] . '</p></div>';
                } ?>
            ...
            <div class="dis_flex">
                ...
                <?php if (!empty($row['d_contact'])) {
                    echo '<a href="contact_download.php?id=' . $row['id'] . '"><div class="big_btns">Save to Contacts <i class="fa fa-download"></i></div></a>';
                } ?>
                ...
                <!-- <div class="share_box">
                    ...
                    <a href="https://www.instagram.com/?url=https://<?php echo $_SERVER['HTTP_HOST']; ?>/<?php echo $row['card_id']; ?>"
                        target="_blank">
        ...
        if (!empty($row['d_about_us'])) { ?>
            <div class="card2" id="about_us">
                ...
                ?>
                <?php if (!empty($row['d_about_us'])) {
                    echo '<p>' . $row['d_about_us'] . '</p>';
                } ?>
            </div>
        <?php
        }
        ...
        if (
            !empty($row3["d_gall_img1"]) || !empty($row3["d_gall_img2"]) || !empty($row3["d_gall_img3"]) ||
        ...
        <?php if (!empty($row["d_paytm"]) || !empty($row["d_account_no"]) || !empty($row["d_qr_paytm"]) || !empty($row["d_qr_phone_pay"]) || !empty($row["d_qr_google_pay"]) || !empty($row["d_google_pay"]) || !empty($row["d_phone_pay"]) || !empty($row["d_ac_type"])) { ?>
            ...
            <div class="card2" id="payment">
                ...
                <?php if (!empty($row["d_paytm"])) {
                    echo '<h2>Paytm</h2><p>' . $row['d_paytm'] . '</p>';
                }    ?>
                <?php if (!empty($row["d_google_pay"])) {
                    echo '<h2>Google Pay</h2><p>' . $row['d_google_pay'] . '</p>';
                } ?>
                <?php if (!empty($row["d_phone_pay"])) {
                    echo '<h2>PhonePe</h2><p>' . $row['d_phone_pay'] . '</p>';
                }    ?>

                <?php if (!empty($row["d_account_no"])) {
                    echo '<h3>Bank Account Details</h3>';
                } ?>


                <?php if (!empty($row["d_ac_name"])) {
                    echo '<h2>Name:</h2><p>' . $row['d_ac_name'] . '</p>';
                }    ?>
                <?php if (!empty($row["d_account_no"])) {
                    echo '<h2>Account Number:</h2><p>' . $row['d_account_no'] . '</p>';
                } ?>
                <?php if (!empty($row["d_ifsc"])) {
                    echo '<h2>IFSC Code:</h2><p>' . $row['d_ifsc'] . '</p>';
                } ?>
                <?php if (!empty($row["d_bank_name"])) {
                    echo '<h2>BANK Name:</h2><p>' . $row['d_bank_name'] . '</p>';
                }    ?>


                <?php if (!empty($row["d_ac_type"])) {
                    echo '<h3>GST Number </h3><h2>GST No:</h2><p>' . $row['d_ac_type'] . '</p>';
                } ?>

                <?php if (!empty($row["d_qr_paytm"])) {
                    echo '<img src="data:image/*;base64,' . base64_encode($row["d_qr_paytm"]) . '" alt="Paytm QR">';
                }    ?>
                <?php if (!empty($row["d_qr_google_pay"])) {
                    echo '<img src="data:image/*;base64,' . base64_encode($row["d_qr_google_pay"]) . '" alt="Google Pay QR">';
                }    ?>
                <?php if (!empty($row["d_qr_phone_pay"])) {
                    echo '<img src="data:image/*;base64,' . base64_encode($row["d_qr_phone_pay"]) . '" alt="PhonePe QR">';
                }    ?>
        ...
        <div class="menu_bottom">
            <div class="menu_container">
                ...
                if (!empty($row2["d_pro_img1"]) || !empty($row2["d_pro_img2"]) || !empty($row2["d_pro_img3"]) || !empty($row2["d_pro_img4"]) || !empty($row2["d_pro_img5"]) || !empty($row2["d_pro_img6"]) || !empty($row2["d_pro_img7"]) || !empty($row2["d_pro_img8"]) || !empty($row2["d_pro_img9"]) || !empty($row2["d_pro_img10"]) || !empty($row2["d_pro_img11"]) || !empty($row2["d_pro_img12"]) || !empty($row2["d_pro_img13"]) || !empty($row2["d_pro_img14"]) || !empty($row2["d_pro_img15"]) || !empty($row2["d_pro_img16"]) || !empty($row2["d_pro_img17"]) || !empty($row2["d_pro_img18"]) || !empty($row2["d_pro_img19"]) || !empty($row2["d_pro_img20"])) { ?>
                    ...
                    <div class="menu_item" onclick="location.href='#product_services'"><i class="fa fa-ticket"></i> Product &
                        Services</div>
                <?php
                }
                if (
    ...
    <?php
    // echo '<div class="row_contd" style="position: fixed; top: 86px; right: 20px;">
    //         <a href="create_card.php?card_number=' . htmlspecialchars($cardrow['id']) . '">
    ...
    //         echo '<div class="row_contd"><a href="https://' . $_SERVER['HTTP_HOST'] . '/' . htmlspecialchars($row['card_id']) . '" target="_blank">' . htmlspecialchars($row['id']) . '</a></div>';
    //         echo '<div class="row_contd">' . htmlspecialchars($row['d_comp_name']) . ' <i class="fa fa-external-link"></i></div>';
    //         echo '<div class="row_contd" id="' . htmlspecialchars($row['d_payment_status']) . '">' . htmlspecialchars($row['d_payment_status']) . '</div>';
    //         echo '<div class="row_contd" id="' . htmlspecialchars($row['d_card_status']) . '">' . ($row['d_card_status'] == 'Active' ? 'Active' : 'Inactive') . '</div>';
    //         echo '<div class="row_contd">' . date("d-M-Y", strtotime($row['uploaded_date'])) . '</div>';
    //         echo '<div class="row_contd">
    //                 <a href="https://api.whatsapp.com/send?text=https://' . $_SERVER['HTTP_HOST'] . '/' . htmlspecialchars($row['card_id']) . '" target="_blank"><i class="fa fa-whatsapp"></i></a>
    //                 <a href="https://www.facebook.com/sharer/sharer.php?u=https://' . $_SERVER['HTTP_HOST'] . '/' . htmlspecialchars($row['card_id']) . '" target="_blank"><i class="fa fa-facebook"></i></a>
    //               </div>';
    //         echo '<div class="row_contd"><a href="create_card.php?card_number=' . htmlspecialchars($row['id']) . '"><i class="fa fa-edit"></i></a></div>';
    //         echo '</li>';
    //     }
    //     echo '</ul>';
    // } else {
