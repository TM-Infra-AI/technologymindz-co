...
// Fetch or Insert into `products` table
$stmt2 = $connect->prepare("SELECT * FROM products WHERE id = ?");
...
if (!$row2) {
    $stmt_insert = $connect->prepare("INSERT INTO products (id) VALUES (?)");
...
<head>
    ...
    <title>E-commerce Product Upload</title>
...
<div class="main3">
    <div class="navigator_up">
        ...
        <a href="create_card5.php"><div class="nav_cont"><i class="fa fa-ticket"></i> Product & Service</div></a>
    ...
    <p class="sug_alert">(At least **1 product image** is required, max size 250 KB)</p>

    <form action="" method="POST" enctype="multipart/form-data">
        <?php 
        for ($m = 1; $m <= 20; $m++) { 
            $product_image = !empty($row2["pro_img$m"]) ? 'data:image/*;base64,'.base64_encode($row2["pro_img$m"]) : 'images/upload.png';
            ?>
            <div class="divider2">
                ...
                <div class="input_box">
                    <input type="file" name="pro_img<?php echo $m; ?>" id="clickMeImage<?php echo $m; ?>" accept="image/*" onchange="previewImage(this, <?php echo $m; ?>);">
                </div>
                <div class="input_box"><p>Product Name</p><input type="text" name="pro_name<?php echo $m; ?>" maxlength="200" value="<?php echo htmlspecialchars($row2["pro_name$m"] ?? ""); ?>"></div>
                <div class="input_box"><p>MRP</p><input type="number" name="pro_mrp<?php echo $m; ?>" max="500000" min="0" value="<?php echo $row2["pro_mrp$m"] ?? ""; ?>"></div>
                <div class="input_box"><p>Selling Price</p><input type="number" name="pro_price<?php echo $m; ?>" max="500000" min="0" value="<?php echo $row2["pro_price$m"] ?? ""; ?>"></div>
            </div>
        <?php } ?>
    ...
    if (isset($_POST['product'])) {
        ...
        for ($x = 1; $x <= 20; $x++) {
            if (!empty($_FILES["pro_img$x"]['tmp_name'])) {
                ...
                $compressedImage = compressImage($_FILES["pro_img$x"]['tmp_name']);
                if ($compressedImage) {
                    $stmt = $connect->prepare("UPDATE products SET pro_img$x=? WHERE id=?");
        ...
        if (!$at_least_one_image) {
            echo '<div class="alert danger">Please upload at least **1 product image** before proceeding.</div>';
        } else {
